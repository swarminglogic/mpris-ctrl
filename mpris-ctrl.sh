...
If this exists as a MPRIS DBus destination of org.mpris.MediaPlayer2.[PLAYER],
then everything should work fine.

When using VLC, PLAYER=vlc is valid if you start VLC with 'vlc --control dbus'
If not, you have to specify the process ID, and set PLAYER=vlc.instance[PID].
...
Notes:
This utility relies entirely on the PLAYER implementation of the MPRIS DBus
interface. Some commands might not work properly, e.g. spotify ignores the
'play' command, does not support org.mpris.MediaPlayer2.PlayList, and volume
...
function execute-command {
    case $1 in
        ...
        volume)
            set -- $@
            if [ $# -eq 2 ] ; then
                dbus-set-player-property Volume variant:double:$2 >/dev/null
            else
